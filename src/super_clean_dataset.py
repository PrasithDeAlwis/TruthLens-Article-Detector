"""
Super clean dataset - remove all common words for maximum clarity
"""
...
def super_clean_dataset():
    """Make dataset extremely clean by removing all common words"""
    ...
    print("=" * 80)
    print("SUPER CLEANING DATASET - REMOVING ALL COMMON WORDS")
    print("=" * 80)
    ...
    print(f"✓ Balanced: {len(df)} articles")
    
    # Super aggressive cleaning
    print(f"\n[3/5] SUPER CLEANING text...")
    print("  - Removing ALL common words (of, to, like, we, us, all, etc.)")
    print("  - Removing numbers, punctuation, URLs")
    print("  - Keeping only meaningful content words")
    
    # Extended stopwords list
    stop_words = set(stopwords.words('english'))
    
    # Add more common words to remove
    extra_stopwords = {
    ...
    for i, text in enumerate(df['text']):
        ...
        # Extra cleaning: remove all the extra stopwords
        words = cleaned.split()
        words = [w for w in words if w not in stop_words and len(w) > 2]
        
        super_cleaned.append(' '.join(words))
    
    df['super_cleaned_text'] = super_cleaned
    
    # Remove empty or very short texts
    print(f"\n[4/5] Removing empty/short texts...")
    df['word_count'] = df['super_cleaned_text'].str.split().str.len()
    df = df[df['word_count'] >= 5]  # At least 5 meaningful words
    df = df.drop('word_count', axis=1)
    
    print(f"✓ Remaining: {len(df)} articles")
    
    # Final dataset
    final_df = df[['text', 'super_cleaned_text', 'label']]
    final_df.columns = ['text', 'cleaned_text', 'label']
    
    # Save
    print(f"\n[5/5] Saving super cleaned dataset...")
    output_path = os.path.join(data_dir, 'super_clean_dataset.csv')
    ...
    # Show examples
    print(f"\n" + "=" * 80)
    print("EXAMPLES - BEFORE vs AFTER")
    print("=" * 80)
    ...
    print(f"  Avg words (super clean): {final_df['cleaned_text'].str.split().str.len().mean():.0f}")
    print(f"  Word reduction: {(1 - final_df['cleaned_text'].str.split().str.len().mean() / final_df['text'].str.split().str.len().mean()) * 100:.1f}%")
    
    print(f"\n" + "=" * 80)
    print("✅ DATASET IS NOW SUPER CLEAN!")
    print("=" * 80)
    print(f"\nTrain with super clean data:")
    print(f"  python src/quick_train_improved.py --data data/super_clean_dataset.csv")
    print("=" * 80)
